...
const bezierUtilsTypeScript = `const kSTS = 11,
kSSS = 1 / (kSTS - 1),
map = (n: number, t: number, S: number, r: number, u: number) =>
	((n - t) * (u - r)) / (S - t) + r,
A = (n: number, t: number) => 1 - 3 * t + 3 * n,
B = (n: number, t: number) => 3 * t - 6 * n,
C = (n: number) => 3 * n,
cB = (n: number, t: number, S: number) => ((A(t, S) * n + B(t, S)) * n + C(t)) * n,
gS = (n: number, t: number, S: number) => 3 * A(t, S) * n * n + 2 * B(t, S) * n + C(t),
bS = (n: number, t: number, S: number, r: number, u: number) => {
	let c,
		o,
		e = 0
	do {
...
nRI = (n: number, t: number, S: number, r: number) => {
	for (let u = 0; u < 4; ++u) {
		const u = gS(t, S, r)
		if (0 === u) return t
		t -= (cB(t, S, r) - n) / u
...
b = (n: number, t: number, S: number, r: number) => {
	if (!(0 <= n && n <= 1 && 0 <= S && S <= 1)) return
